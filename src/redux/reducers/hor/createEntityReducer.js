export default function createEntityReducer(entityName = '') {
	...
	return (state = {}, { type, payload }) => {
		console.log(type)
		console.log(entityConstant)
//		console.log(`i'm the payload in the entity rude`, payload)
		switch (type) {
		...
		case `FETCH_${entityConstant}_PENDING`:
			return { ...state, [entityName]: payload };
		case `FETCH_${entityConstant}_SUCCESS`:
			return { ...state, [entityName]: payload };
		...
		case `CREATE_${entityConstant}_PENDING`:
			return { ...state, [entityName]: payload };
		case `CREATE_${entityConstant}_SUCCESS`:
		...
		case `UPDATE_${entityConstant}_PENDING`:
			return { ...state, [entityName]: payload };
			case `UPDATE_${entityConstant}_SUCCESS`:
		...
		case `DELETE_${entityConstant}_PENDING`:
			return { ...state, [entityName]: payload };
		case `DELETE_${entityConstant}_SUCCESS`:
			return { ...state, [entityName]: payload };
